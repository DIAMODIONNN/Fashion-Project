  ...
  return (
    <div className={`container mx-auto p-4 ${"bg-white"}`}>
      <div className="flex justify-center items-start mt-12">
        <h1
          className={`font-bold text-2xl border-b-2 ${"border-[#48CFCB]"} text-[#48CFCB]`}
        >
          P R O D U C T S
        </h1>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mt-6">
        {products.map((product) => (
          <div
            key={product.id}
            className={`rounded-lg shadow-lg overflow-hidden transition-transform transform hover:scale-105 ${"bg-[#F5F5F5]"}`}
          >
            <div
              className={`h-40 w-100 flex justify-center items-center pb-5 ${"bg-[#F5F5F5]"}`}
            >
              <img
                src={product.image}
                alt={product.title}
            ...
            <div className={`p-4 ${"text-black"}`}>
              ...
              <div className="flex justify-between">
                ...
                <button
                  ...
                  className={`mt-2 p-2 ${" text-[#229799]"}`}
                >
